...
function libraryGcodeEditor(file) {
  ...
  return `<article class="library-gcode-row ${file.active === false ? 'inactive' : ''}"><img src="${escape(file.thumbnail?.url || '')}" alt="Pré-visualização de ${escape(file.original_name)}"><div class="library-gcode-main"><div class="library-gcode-title"><div><strong>${escape(file.original_name)}</strong><small>${productionFileKind(file)} · ${Math.max(1, Math.round(Number(file.size_bytes || 0) / 1024))} KB · ${file.active === false ? 'Inativo' : 'Ativo'}</small>${productionMaterialMarkup(meta)}${warnings}</div><button class="compact danger" data-delete-file="${escape(file.id)}">Remover</button></div><form class="gcode-edit-form" data-edit-gcode="${escape(file.id)}"><label>Impressora / perfil<input name="printer_model" required list="printer-model-list" value="${escape(file.printer_model || '')}" placeholder="Ex.: Bambu Lab P1S"></label><label>Material principal<input name="material" required value="${escape(meta.material || '')}" placeholder="PETG"></label><label>Cor principal<input name="color" required value="${escape(meta.color || '')}" placeholder="Preto"></label><label>Bico (mm)<input name="nozzle" required type="number" min="0.1" max="2" step="0.1" value="${escape(meta.nozzle || '')}"></label><label>Peças por impressão<input name="quantity" required type="number" min="1" step="1" value="${escape(meta.quantity || 1)}"></label><label>Utilização<select name="active"><option value="true" ${file.active !== false ? 'selected' : ''}>Ativo</option><option value="false" ${file.active === false ? 'selected' : ''}>Inativo</option></select></label><button class="compact secondary" type="submit">Guardar dados</button></form></div></article>`;
}
function renderFiles() {
  $('file-grid').innerHTML = libraryParts.length ? libraryParts.map((part) => `<article class="library-part-card"><header class="library-part-header"><div><p class="eyebrow">PEÇA · ${part.gcodes?.length || 0} VARIANTE(S) DE PRODUÇÃO</p><h2>${escape(part.name)}</h2><p>${escape(part.description || 'Sem descrição')}</p></div>${part.gcodes?.length ? '' : `<button class="compact danger" data-delete-library-part="${escape(part.id)}">Remover peça</button>`}</header><div class="library-gcode-list">${part.gcodes?.length ? part.gcodes.map(libraryGcodeEditor).join('') : '<p class="empty">Adiciona o primeiro G-code ou 3MF desta peça.</p>'}</div><form class="part-upload-form" data-upload-part="${escape(part.id)}"><label>Ficheiro de produção<input name="gcode" type="file" accept=".gcode,.gco,.3mf" required></label><label>Impressora / perfil<input name="printer_model" required list="printer-model-list" placeholder="Ex.: Bambu Lab P1S"></label><label>Material principal<input name="material" placeholder="PETG"></label><label>Cor principal<input name="color" placeholder="Preto"></label><label>Bico (mm)<input name="nozzle" type="number" min="0.1" max="2" step="0.1" placeholder="0.4"></label><label>Peças por impressão<input name="quantity" type="number" min="1" step="1" placeholder="1"></label><button class="compact" type="submit">Adicionar ficheiro</button><small>Aceita G-code e 3MF. Num 3MF, o portal lê automaticamente materiais, cores, bico e quantidade quando o slicer os gravou.</small></form></article>`).join('') : '<p class="empty">Ainda não existem peças. Cria a primeira peça e adiciona variantes G-code ou 3MF.</p>';
...
for (const id of ['order-form', 'project-form', 'printer-form', 'spool-form', 'customer-form', 'library-part-form']) $(id).addEventListener('submit', async (event) => {
  ...
  try {
    if (id === 'spool-form') {
      ...
      toast(`${result.total} ${result.total === 1 ? 'linha guardada' : 'linhas guardadas'} · ${Math.round(result.added_weight)} g adicionados ao stock.`);
      await update();
      return;
    }
