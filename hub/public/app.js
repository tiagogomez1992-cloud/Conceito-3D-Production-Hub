...
function resetStockRows() { const rows = $('stock-entry-rows'); if (rows) rows.innerHTML = stockRowMarkup(); }
function addStockRow(values = {}) { const rows = $('stock-entry-rows'); if (!rows) return; rows.insertAdjacentHTML('beforeend', stockRowMarkup(values)); rows.lastElementChild?.querySelector('[data-stock-field="material"]')?.focus(); }
function stockEntriesFromForm() { return [...document.querySelectorAll('#stock-entry-rows [data-stock-row]')].map((row) => Object.fromEntries([...row.querySelectorAll('[data-stock-field]')].map((field) => [field.dataset.stockField, field.value.trim()]))); }
...
function printerModelChoices(selected = '') {
  ...
  const hasSelected = models.some((model) => model === selected);
  const unavailable = selected && !hasSelected ? `<option value="${escape(selected)}" selected>${escape(selected)} · perfil já não registado</option>` : '';
  return `<option value="">${models.length ? 'Selecionar perfil' : 'Sem modelos de impressora registados'}</option>${unavailable}${models.map((model) => `<option value="${escape(model)}" ${model === selected ? 'selected' : ''}>${escape(model)}</option>`).join('')}`;
}
function stockMaterialRows() {
  const source = latest.stock?.length ? latest.stock : latest.spools;
  const items = source.map((item) => ({
    material: String(item.material || item.filament?.material || '').trim(),
    color: String(item.color_name || item.color || item.filament?.color || '').trim(),
    remaining: Number(item.remaining_weight || 0),
  })).filter((item) => item.material && item.remaining > 0);
  const unique = new Map();
  items.forEach((item) => unique.set(`${item.material.toLocaleLowerCase('pt-PT')}|${item.color.toLocaleLowerCase('pt-PT')}`, item));
  return [...unique.values()].sort((left, right) => `${left.material} ${left.color}`.localeCompare(`${right.material} ${right.color}`, 'pt-PT'));
}
function stockMaterialChoices(selected = '') {
  const materials = [...new Map(stockMaterialRows().map((item) => [item.material.toLocaleLowerCase('pt-PT'), item.material])).values()];
  const hasSelected = materials.some((item) => item.toLocaleLowerCase('pt-PT') === String(selected).toLocaleLowerCase('pt-PT'));
  const unavailable = selected && !hasSelected ? `<option value="${escape(selected)}" selected>${escape(selected)} · sem stock atual</option>` : '';
  return `<option value="">${materials.length ? 'Selecionar material em stock' : 'Sem material em stock'}</option>${unavailable}${materials.map((material) => `<option value="${escape(material)}" ${material.toLocaleLowerCase('pt-PT') === String(selected).toLocaleLowerCase('pt-PT') ? 'selected' : ''}>${escape(material)}</option>`).join('')}`;
}
function stockColorChoices(material = '', selected = '') {
  const key = String(material).trim().toLocaleLowerCase('pt-PT');
  const colors = [...new Map(stockMaterialRows().filter((item) => item.material.toLocaleLowerCase('pt-PT') === key && item.color && item.color.toLocaleLowerCase('pt-PT') !== 'sem cor').map((item) => [item.color.toLocaleLowerCase('pt-PT'), item.color])).values()];
  const hasSelected = colors.some((item) => item.toLocaleLowerCase('pt-PT') === String(selected).toLocaleLowerCase('pt-PT'));
  const unavailable = selected && !hasSelected ? `<option value="${escape(selected)}" selected>${escape(selected)} · sem stock atual</option>` : '';
  const prompt = !material ? 'Escolhe primeiro o material' : colors.length ? 'Selecionar cor em stock' : 'Sem cores em stock para este material';
  return `<option value="">${prompt}</option>${unavailable}${colors.map((color) => `<option value="${escape(color)}" ${color.toLocaleLowerCase('pt-PT') === String(selected).toLocaleLowerCase('pt-PT') ? 'selected' : ''}>${escape(color)}</option>`).join('')}`;
}
function refreshLibraryFilePickers(root = document) {
  root.querySelectorAll('[data-file-material-select]').forEach((material) => {
    const selectedMaterial = material.value;
    material.innerHTML = stockMaterialChoices(selectedMaterial);
    const form = material.closest('.part-upload-form, .gcode-edit-form');
    const color = form?.querySelector('[data-file-color-select]');
    if (!color) return;
    const selectedColor = color.value;
    color.innerHTML = stockColorChoices(material.value, selectedColor);
    const hasStockColor = stockMaterialRows().some((item) => item.material.toLocaleLowerCase('pt-PT') === material.value.toLocaleLowerCase('pt-PT') && item.color && item.color.toLocaleLowerCase('pt-PT') !== 'sem cor');
    color.disabled = !material.value || (!hasStockColor && !color.value);
  });
  root.querySelectorAll('[data-file-printer-model]').forEach((model) => {
    const selectedModel = model.value;
    model.innerHTML = printerModelChoices(selectedModel);
  });
}
function productionFileKind(file) { return /\.3mf$/i.test(String(file?.original_name || '')) ? '3MF' : 'G-code'; }
function productionMaterials(meta = {}) {
  const entries = Array.isArray(meta.materials) && meta.materials.length ? meta.materials : [{ material: meta.material, color: meta.color, color_hex: meta.color }];
  return entries.filter((entry) => entry?.material || entry?.color).map((entry) => `${entry.material || 'Material'}${entry.color ? ` · ${entry.color}` : ''}`);
}
function productionMaterialMarkup(meta = {}) {
  const materials = productionMaterials(meta);
  return materials.length ? `<small class="production-materials">${materials.map((entry) => `<span>${escape(entry)}</span>`).join('')}</small>` : '';
}
function libraryGcodeEditor(file) {
  const meta = file.metadata || {};
  const warnings = Array.isArray(meta.warnings) && meta.warnings.length ? `<small class="production-file-warning">${escape(meta.warnings.join(' '))}</small>` : '';
  return `<article class="library-gcode-row ${file.active === false ? 'inactive' : ''}"><img src="${escape(file.thumbnail?.url || '')}" alt="Pré-visualização de ${escape(file.original_name)}"><div class="library-gcode-main"><div class="library-gcode-title"><div><strong>${escape(file.original_name)}</strong><small>${productionFileKind(file)} · ${Math.max(1, Math.round(Number(file.size_bytes || 0) / 1024))} KB · ${file.active === false ? 'Inativo' : 'Ativo'}</small>${productionMaterialMarkup(meta)}${warnings}</div><button class="compact danger" data-delete-file="${escape(file.id)}">Remover</button></div><form class="gcode-edit-form" data-edit-gcode="${escape(file.id)}"><label>Impressora / perfil<select name="printer_model" required data-file-printer-model>${printerModelChoices(file.printer_model || '')}</select></label><label>Material principal<select name="material" required data-file-material-select>${stockMaterialChoices(meta.material || '')}</select></label><label>Cor principal<select name="color" required data-file-color-select ${!meta.material ? 'disabled' : ''}>${stockColorChoices(meta.material || '', meta.color || '')}</select></label><label>Bico (mm)<input name="nozzle" required type="number" min="0.1" max="2" step="0.1" value="${escape(meta.nozzle || '')}"></label><label>Peças por impressão<input name="quantity" required type="number" min="1" step="1" value="${escape(meta.quantity || 1)}"></label><label>Utilização<select name="active"><option value="true" ${file.active !== false ? 'selected' : ''}>Ativo</option><option value="false" ${file.active === false ? 'selected' : ''}>Inativo</option></select></label><button class="compact secondary" type="submit">Guardar dados</button></form></div></article>`;
}
function renderFiles() {
  $('file-grid').innerHTML = libraryParts.length ? libraryParts.map((part) => `<article class="library-part-card"><header class="library-part-header"><div><p class="eyebrow">PEÇA · ${part.gcodes?.length || 0} VARIANTE(S) DE PRODUÇÃO</p><h2>${escape(part.name)}</h2><p>${escape(part.description || 'Sem descrição')}</p></div>${part.gcodes?.length ? '' : `<button class="compact danger" data-delete-library-part="${escape(part.id)}">Remover peça</button>`}</header><div class="library-gcode-list">${part.gcodes?.length ? part.gcodes.map(libraryGcodeEditor).join('') : '<p class="empty">Adiciona o primeiro G-code ou 3MF desta peça.</p>'}</div><form class="part-upload-form" data-upload-part="${escape(part.id)}"><label>Ficheiro de produção<input name="gcode" type="file" accept=".gcode,.gco,.3mf" required></label><label>Impressora / perfil<select name="printer_model" required data-file-printer-model>${printerModelChoices()}</select></label><label>Material principal<select name="material" required data-file-material-select>${stockMaterialChoices()}</select></label><label>Cor principal<select name="color" required data-file-color-select disabled>${stockColorChoices()}</select></label><label>Bico (mm)<input name="nozzle" type="number" min="0.1" max="2" step="0.1" placeholder="0.4"></label><label>Peças por impressão<input name="quantity" type="number" min="1" step="1" placeholder="1"></label><button class="compact" type="submit">Adicionar ficheiro</button><small>Modelos vêm das impressoras registadas. Material e cor são escolhidos apenas entre o stock disponível.</small></form></article>`).join('') : '<p class="empty">Ainda não existem peças. Cria a primeira peça e adiciona variantes G-code ou 3MF.</p>';
  refreshLibraryFilePickers($('file-grid'));
}
...
document.addEventListener('click', async (event) => {
  ...
  if (open) { $(open.dataset.openForm).classList.remove('hidden'); if (open.dataset.openForm === 'printer-form') { $('printer-form').reset(); resetPrinterFormMode(); setPrinterCatalogSelection(); } if (open.dataset.openForm === 'order-form') { $('order-form').reset(); resetOrderPdfAnalysis(); } if (open.dataset.openForm === 'spool-form') resetStockRows(); if (open.dataset.openForm === 'quick-dispatch-form') { resetQuickDispatchForm(); } }
  if (close) { $(close.dataset.closeForm).classList.add('hidden'); if (close.dataset.closeForm === 'customer-form') resetTemplateForm(); if (close.dataset.closeForm === 'printer-form') { $('printer-form').reset(); resetPrinterFormMode(); setPrinterCatalogSelection(); } if (close.dataset.closeForm === 'order-form') { $('order-form').reset(); resetOrderPdfAnalysis(); } if (close.dataset.closeForm === 'spool-form') resetStockRows(); if (close.dataset.closeForm === 'quick-dispatch-form') resetQuickDispatchForm(); }
  if (event.target.closest('#add-stock-row')) { addStockRow(); return; }
  const removeStockRow = event.target.closest('[data-remove-stock-row]');
  if (removeStockRow) { const rows = [...document.querySelectorAll('#stock-entry-rows [data-stock-row]')]; if (rows.length === 1) { toast('Mantém pelo menos uma linha de stock.', 'error'); } else removeStockRow.closest('[data-stock-row]')?.remove(); return; }
...
function refreshStockLineTotal(event) {
  if (!event.target.matches('[data-stock-field="spool_weight"], [data-stock-field="spool_count"]')) return;
  updateStockRowTotal(event.target.closest('[data-stock-row]'));
}
document.addEventListener('input', refreshStockLineTotal);
document.addEventListener('change', refreshStockLineTotal);
document.addEventListener('change', async (event) => {
  const fileMaterial = event.target.closest('[data-file-material-select]');
  if (fileMaterial) {
    const form = fileMaterial.closest('.part-upload-form, .gcode-edit-form');
    const color = form?.querySelector('[data-file-color-select]');
    if (color) {
      color.innerHTML = stockColorChoices(fileMaterial.value);
      color.disabled = !fileMaterial.value || !stockMaterialRows().some((item) => item.material.toLocaleLowerCase('pt-PT') === fileMaterial.value.toLocaleLowerCase('pt-PT') && item.color && item.color.toLocaleLowerCase('pt-PT') !== 'sem cor');
    }
    return;
  }
...
for (const id of ['order-form', 'project-form', 'printer-form', 'spool-form', 'customer-form', 'library-part-form']) $(id).addEventListener('submit', async (event) => {
  ...
  try {
    if (id === 'spool-form') {
      const entries = stockEntriesFromForm();
      const result = await api('/api/spools/bulk', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ entries }) });
      resetStockRows(); formElement.classList.add('hidden');
      const unitCount = Number(result.added_spool_count || 0);
      toast(`${result.total} ${result.total === 1 ? 'artigo guardado' : 'artigos guardados'} · ${unitCount} ${unitCount === 1 ? 'bobine fechada adicionada' : 'bobines fechadas adicionadas'}.`);
      await update();
      return;
    }
