...
rollback() {
  ...
  docker compose -f "$APP_DIR/compose.yml" up -d --build
}
...
if ! docker compose -f "$APP_DIR/compose.yml" up -d --build; then
  rollback
  exit 1
fi

sleep 5
if ! docker compose -f "$APP_DIR/compose.yml" ps --status running --services | grep -qx web; then
  rollback
  exit 1
fi
